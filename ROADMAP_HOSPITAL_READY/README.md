...
> **Estado a 11 de septiembre de 2026:** Fases 0 y 1 completadas y la oleada 1 de Fase 2 integrada en `main` (ADRs 0130, 0140, 0150 y 0160). RBAC/ABAC, gobierno RAG y SSO OIDC continúan en curso; el cierre live de RLS y seguridad depende del propietario. El orden y detalle están en [15-implementation-phases.md](15-implementation-phases.md) y `docs/ESTADO_ACTUAL.md`.
...
Lo que existe tras Fase 1 y la oleada 1 de Fase 2 (paquete `chathce/`): Clinical Data Gateway (`ClinicalDataProvider` + `ScopeGuard`, adapter MIMIC), Clinical AI Gateway (`ModelGateway` + `ToolRegistry` + `ToolPolicy`), RLS/clave clínica versionados, kill switch/circuit breaker, minimización PHI, schemas `Evidence`/`Claim` y FastAPI con JSON y SSE. Faltan: Evidence Engine completo, Policy Engine con RBAC/ABAC, gobierno RAG, SSO OIDC, FHIR/SMART y el frontend React.
...
| [05-identity-authorization-multitenancy.md](05-identity-authorization-multitenancy.md) | SSO, RBAC/ABAC, aislamiento | P0 | ✅ RLS/JWT/clave readonly versionados; 🔄 RBAC/ABAC y SSO OIDC en curso; ⏳ live |
| [06-privacy-phi-security.md](06-privacy-phi-security.md) | PHI, minimización, cifrado y DLP | P0 | ✅ logging, minimización y detección PHI locales; ⏳ DLP/cifrado/política de egreso |
| [07-agent-safety-tooling.md](07-agent-safety-tooling.md) | Seguridad agentic, tools y sandbox | P0 | ✅ P0 y kill switch/circuit breaker; ⏳ resiliencia distribuida |
...
| [11-evaluation-red-team.md](11-evaluation-red-team.md) | Evaluación clínica, seguridad y red team | P0 | ✅ gate CI y 78 controles offline; ⏳ runner live/red team continuo |
| [12-observability-audit-resilience.md](12-observability-audit-resilience.md) | Auditoría, observabilidad y continuidad | P0/P1 | ✅ audit trail, timeouts, kill switch/circuit breaker; ⏳ dashboards y estado compartido |
...
| [15-implementation-phases.md](15-implementation-phases.md) | Orden recomendado de ejecución | — | Fases 0–1 ✅ · F2 oleada 1 ✅ · oleada 2 🔄 |
...
| exista aislamiento estricto hospital/usuario/paciente/episodio/sesión | 🟡 RLS y concesión usuario-paciente versionadas; aplicar/verificar live; tenant y relación asistencial pendientes |
...
| PHI se minimice antes de llegar al modelo | ✅ catálogo por DTO y detector local (ADR 0160); ⏳ DLP/política externa |
...
| exista una batería amplia de evaluaciones clínicas y adversariales | ✅ 78 controles offline, gate CI y 0 violaciones críticas; ⏳ ejecución live/red team continuo |
| exista un modo degradado y kill switch de IA | ✅ kill switch y circuit breaker por modelo; ⏳ estado distribuido |
