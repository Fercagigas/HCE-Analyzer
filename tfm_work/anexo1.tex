...
El repositorio del proyecto se encuentra disponible en GitHub: \url{https://github.com/Fercagigas/HCE-Analyzer}.
...
La herramienta de base de datos proporciona acceso seguro y de solo lectura al dataset \acs{MIMIC}-IV-\acs{ED}. Implementa validación multicapa de consultas \ac{SQL} y soporta cinco tipos de consulta predefinidos.
...
\begin{table}[H]
\centering
\caption{Reglas de validación de seguridad \acs{SQL} implementadas en la Database Tool.}
...
\begin{table}[H]
\centering
\caption{Documentos clínicos indexados en el sistema \acs{RAG} con descripción detallada.}
...
% ====================================================================
\section{Pipeline \acs{RAG}: detalles técnicos completos}
...
donde $R = \{\text{vectorial}, \text{léxica}\}$ y $k = 60$ es la constante de suavizado que balancea la influencia de las primeras posiciones. El valor $k=60$ es el estándar recomendado en la literatura \parencite{cormack2009rrf} y fue adoptado sin necesidad de ajuste adicional.
...
\begin{table}[H]
\centering
\caption{Evolución de métricas \acs{RAGAS} en el golden set DB (40 preguntas sobre \acs{MIMIC}-IV-\acs{ED}).}
...
\begin{table}[H]
\centering
\caption{Evolución de métricas \acs{RAGAS} en el golden set \acs{RAG} (30 preguntas sobre guías clínicas \acs{UCI}).}
...
\subsection{Capa 3: validación \acs{SQL} multicapa}
...
Supabase Auth proporciona autenticación JWT con RLS habilitado en las 13 tablas del sistema (6 tablas \acs{MIMIC}-IV-\acs{ED} + 7 tablas de aplicación). Las políticas RLS garantizan que cada usuario solo puede acceder a sus propias sesiones, mensajes y preferencias.
...
Limitaciones & \acs{MIMIC}-IV-\acs{ED} es dataset demo con 222 pacientes; solo urgencias; sin laboratorio, imagen ni cirugía \\
...
\subsection{Sandbox \acs{AST} para visualizaciones}
...
\subsection{Tablas \acs{MIMIC}-IV-ED}
...
\subsection{Tabla Vectorial \acs{RAG} (\texttt{rag\_chunks})}
...
% ====================================================================
\section{Gestor de modelos \acs{LLM}: implementación detallada}
...
\begin{table}[H]
\centering
\caption{Configuración completa de la cadena de modelos \acs{LLM}.}
...
2 & CONTEXTO & Dataset \acs{MIMIC}-IV-\acs{ED}: 222 pacientes, 6 tablas, urgencias hospitalarias \\
...
Interfaz & Streamlit~\parencite{streamlit2019} & Chat interactivo \\
Agente & LangChain 1.2.7~\parencite{chase2022langchain} & Orquestación y herramientas \\
...
Base de datos & Supabase PostgreSQL 15~\parencite{supabase2020,postgresql2024} & \acs{MIMIC}-IV-\acs{ED} + aplicación \\
Vector store & pgvector 0.8.0~\parencite{pgvector2024} & Embeddings \ac{RAG} \\
Cliente BD & supabase-py & Acceso desde Python \\
Embeddings & sentence-transformers 5.2.2~\parencite{reimers2019sentencebert} & all-MiniLM-L6-v2 (384d) \\
Evaluación & \ac{RAGAS} 0.4.3 & Evaluación pipeline \ac{RAG} \\
Visualización & Plotly~\parencite{plotly2015} & Gráficas interactivas \\
Datos & pandas / numpy & Procesamiento de datos \\
PDF & Docling / PyPDF2 & Extracción de texto \\
DOCX & python-docx & Procesamiento Word \\
Validación & Pydantic~\parencite{pydantic2023} & Validación de entradas \\
