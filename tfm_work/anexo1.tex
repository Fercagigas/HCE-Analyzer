...
Este patrón \textit{Adapter} permite que cada herramienta defina su esquema Pydantic de parámetros (\texttt{args\_schema}), que Claude utiliza directamente para generar los argumentos JSON de invocación.

\subsection{Database Tool (\texttt{query\_mimic\_database})}
\label{sec:appendix_dbtool}

La herramienta de base de datos proporciona acceso seguro y de solo lectura al dataset MIMIC-IV-ED. Implementa validación multicapa de consultas SQL y soporta cinco tipos de consulta predefinidos.
...
\addlinespace
\texttt{diagnoses} & \texttt{icd\_code} o \texttt{icd\_title} & Búsqueda de diagnósticos por código ICD o título & 1000 filas \\
\addlinespace
...
\begin{table}[H]
\centering
\caption{Reglas de validación de seguridad SQL implementadas en la Database Tool.}
...
La herramienta RAG proporciona búsqueda semántica en documentos clínicos indexados. Acepta tres parámetros: \texttt{query} (obligatorio, la consulta en lenguaje natural), \texttt{specialty} (opcional, filtro por especialidad médica) y \texttt{top\_k} (opcional, por defecto 8).

\subsubsection{Pipeline Interno}

El flujo de ejecución del RAG Tool sigue siete etapas secuenciales:

\begin{enumerate}[nosep, leftmargin=*]
  \item \textbf{Recepción}: Validación de parámetros con Pydantic.
  \item \textbf{Augmentación}: \texttt{QueryAugmenter} genera 3 consultas alternativas con Claude Haiku (Multi-Query, temperatura 0.4) y un documento hipotético (HyDE, temperatura 0.2).
...
\begin{table}[H]
\centering
\caption{Documentos clínicos indexados en el sistema RAG con descripción detallada.}
...
\addlinespace
Estándares y Recomendaciones para UCI & Ministerio de Sanidad de España. Ratios médico-paciente, niveles asistenciales, indicadores de calidad, seguridad del paciente en UCI. \\
\addlinespace
El Libro de la UCI & Paul L. Marino, 3.ª edición. Referencia clínica completa de cuidados críticos: ventilación mecánica, sepsis, hemostasia, nutrición en UCI. \\
\bottomrule
\end{tabularx}
\end{table}
...
El RAG Tool formatea la salida para Claude con bloques delimitados por documento:
...
El campo \texttt{raw\_observation} preserva el texto completo sin truncar, permitiendo que el evaluador RAGAS parsee directamente los contextos recuperados.
...
\texttt{acuity\_pie} & Distribución de niveles de acuidad ESI \\
\texttt{medication\_bar} & Frecuencia de medicamentos administrados \\
\texttt{diagnosis\_treemap} & Jerarquía de diagnósticos ICD \\
...
Cuando no hay plantilla disponible (aproximadamente 10\% de los casos), el sistema escala a Claude Sonnet 4.5 para generar código Plotly personalizado. El código generado se valida mediante parsing AST antes de ejecutarse en un sandbox aislado con timeout de 10 segundos.
...
% ====================================================================
\section{Pipeline RAG: detalles técnicos completos}
\label{sec:appendix_rag}
% ====================================================================

Esta sección complementa la descripción del pipeline RAG presentada en la Sección~\ref{sec:rag} del capítulo principal, proporcionando todos los detalles técnicos de implementación.
...
Truncado de contenido & 1500 chars & Máximo de caracteres pasados al LLM por documento \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}[nosep, leftmargin=*]
  \item \textbf{Búsqueda vectorial} (pgvector): Similitud coseno entre el embedding de la consulta (384d) y los embeddings de los chunks hijo. Utiliza índice HNSW para búsqueda ANN en tiempo sublineal.
  \item \textbf{Búsqueda full-text} (tsvector): Búsqueda léxica con stemming en español (\texttt{spanish} configuration). Captura coincidencias exactas de términos médicos, códigos ICD, nombres de fármacos.
\end{itemize}

La fusión RRF combina los rankings sin requerir calibración de pesos:
...
Durante la evaluación con el framework RAGAS, se identificaron seis causas raíz que impedían el correcto funcionamiento del pipeline:

\subsubsection{Causa 1: Desalineación del System Prompt}

El system prompt definía cuándo usar \texttt{search\_clinical\_documents} con términos de ``urgencias y emergencias'', pero los documentos indexados versan sobre \textbf{UCI y Medicina Intensiva}. El agente no reconocía que debía buscar en los documentos para preguntas como ``¿Cuántas camas tiene la UCI?''.

\textbf{Solución:} Se reescribió la sección de selección de herramientas para incluir explícitamente UCI, cuidados críticos y medicina intensiva, listar los documentos reales indexados, y añadir 8 ejemplos concretos de preguntas que deben usar la herramienta. La directiva pasó de ``considera usar'' a ``SIEMPRE invoca''.
...
El método \texttt{format\_output} del RAG Tool truncaba el contenido de cada documento a 600 caracteres. Los parent chunks ($\sim$3000 chars) quedaban cortados, impidiendo que el LLM encontrara datos específicos.
...
\subsubsection{Causa 6: Contexto RAG No Llegaba a RAGAS}

El campo \texttt{raw\_output} se truncaba a 1000 caracteres en \texttt{\_extract\_tool\_result}, y el evaluador RAGAS intentaba parsear un campo \texttt{raw\_observation} que no existía.
...
\begin{table}[H]
\centering
\caption{Evolución de métricas RAGAS en el golden set DB (40 preguntas sobre MIMIC-IV-ED).}
...
\begin{table}[H]
\centering
\caption{Evolución de métricas RAGAS en el golden set RAG (30 preguntas sobre guías clínicas UCI).}
...
\textbf{Faithfulness (RAG: 0.57).} Las respuestas incluyen interpretaciones clínicas del LLM que van más allá del texto recuperado. Posibles mejoras: reforzar directivas anti-alucinación específicas para respuestas RAG; añadir instrucción ``responde SOLO con lo que aparece en los fragmentos''.

\textbf{Answer Relevancy (RAG: 0.48).} Las respuestas son extensas y añaden contexto no solicitado. Posibles mejoras: directiva de concisión específica; limitar longitud cuando la pregunta es directa.

\textbf{Context Precision (RAG: 0.55).} Aproximadamente el 55\% del contexto recuperado es directamente relevante. Posibles mejoras: ajustar el modelo de reranking para el dominio médico en español; aumentar peso de búsqueda léxica para términos técnicos exactos.

\textbf{Context Recall (RAG: 0.49).} Aproximadamente la mitad de la información necesaria se recupera. Posibles mejoras: aumentar \texttt{top\_k} a 10-12 para preguntas directas; mejorar chunking para preservar datos numéricos en contexto.
...
\subsection{Capa 3: validación SQL multicapa}
...
Supabase Auth proporciona autenticación JWT con RLS habilitado en las 13 tablas del sistema (6 tablas MIMIC-IV-ED + 7 tablas de aplicación). Las políticas RLS garantizan que cada usuario solo puede acceder a sus propias sesiones, mensajes y preferencias.

\subsection{Capa 5: audit logging}

Todas las operaciones del sistema se registran con logs estructurados que incluyen: timestamp, session ID, tipo de operación, herramienta invocada, tiempo de ejecución, resultado (éxito/error), y modelo LLM utilizado. Estos logs permiten trazabilidad completa para auditoría y debugging.
...
Citación de fuentes & Siempre indicar herramienta usada y tabla de origen; para RAG, citar documento fuente \\
Incertidumbre & Distinguir datos verificados de interpretaciones; usar ``Según los datos disponibles...'' \\
Limitaciones & MIMIC-IV-ED es dataset demo con 222 pacientes; solo urgencias; sin laboratorio, imagen ni cirugía \\
...
\subsection{Sandbox AST para visualizaciones}

El código Plotly generado por el LLM se ejecuta en un entorno aislado con validación AST:
...
\midrule
SQL Injection & 7 & 7 & 1.000 & Superado \\
...
\texttt{stay\_id} & INT (PK) & Identificador único de la estancia \\
...
\subsection{Tabla Vectorial RAG (\texttt{rag\_chunks})}
...
\texttt{embedding} & \texttt{vector(384)} & Embedding \texttt{all-MiniLM-L6-v2}. Índice HNSW \\
\texttt{fts} & \texttt{tsvector} & Columna generada desde \texttt{content} con config. \texttt{spanish}. Índice GIN \\
...
\texttt{diagnosis} & \texttt{icd\_code} & B-tree & Búsqueda por código ICD \\
...
\texttt{rag\_chunks} & \texttt{embedding} & HNSW & Búsqueda ANN vectorial \\
\texttt{rag\_chunks} & \texttt{fts} & GIN & Full-text en español \\
\bottomrule
\end{tabularx}
\end{table}
...
% ====================================================================
\section{Gestor de modelos LLM: implementación detallada}
...
\begin{table}[H]
\centering
\caption{Configuración completa de la cadena de modelos LLM.}
...
RAG & \texttt{haiku-4-5} & 4000 & 0.1 & 60s & Query augment. \\
\bottomrule
\end{tabularx}
\end{table}
...
2 & CONTEXTO & Dataset MIMIC-IV-ED: 222 pacientes, 6 tablas, urgencias hospitalarias \\
...
7 & HERR. DETALLADAS & Reglas SQL, tipos de consulta, ejemplos de uso correcto e incorrecto \\
...
10 & SELECCIÓN HERR. & Cuándo usar cada herramienta, reglas obligatorias, docs indexados UCI \\
\bottomrule
\end{tabularx}
\end{table}

La sección 10 (SELECCIÓN DE HERRAMIENTAS) fue la más crítica tras las mejoras al pipeline RAG. Incluye la regla fundamental:
...
Y lista explícitamente los tres documentos indexados (Manual del Residente, Estándares UCI, Libro de la UCI) con 8 ejemplos de preguntas que deben activar la herramienta.
...
El \texttt{CacheManager} implementa caché en memoria con cinco tipos especializados, evicción LRU y TTL configurable. Es un Singleton compartido entre todos los componentes.
...
\texttt{llm\_responses} & 300s & Respuestas de Claude & LRU \\
...
\texttt{cache\_ttl\_seconds} & 300s & TTL por defecto \\
\texttt{cache\_cleanup\_interval} & 600s & Intervalo de limpieza \\
Estructura interna & \texttt{OrderedDict} & Evicción LRU eficiente \\
...
Tokens utilizados & Estimación de tokens consumidos por el LLM \\
...
\addlinespace
Strategy & Búsqueda RAG & Estrategias intercambiables: similarity, MMR, hybrid \\
\addlinespace
...
\addlinespace
Chain of Resp. & Fallback LLM & Cadena Haiku 4.5 $\to$ Sonnet 4.5 $\to$ Opus 4 \\
\bottomrule
\end{tabularx}
\end{table}
...
LLM principal & Claude Haiku 4.5 & Agente conversacional \\
LLM viz. & Claude Sonnet 4.5 & Generación de código \\
SDK Anthropic & anthropic 0.77.0 & Cliente API de Claude \\
Base de datos & Supabase PostgreSQL 15~\citep{supabase2020,postgresql2024} & MIMIC-IV-ED + aplicación \\
Vector store & pgvector 0.8.0~\citep{pgvector2024} & Embeddings RAG \\
...
Evaluación & RAGAS 0.4.3 & Evaluación pipeline RAG \\
