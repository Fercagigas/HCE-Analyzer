...
\section{Calidad de respuestas: RAGAS DB}
\label{sec:res_ragas_db}

Las 40~preguntas del \textit{golden set} DB fueron procesadas sin errores (tasa de error: 0\,\%) en 752,3~segundos. La Tabla~\ref{tab:res_ragas_db} presenta las cuatro métricas RAGAS obtenidas.

\begin{table}[H]
\centering
\caption{Métricas RAGAS --- \textit{Golden Set} DB (40~preguntas, 0~errores, 752,3\,s).}
...
\caption{Métricas RAGAS del \textit{golden set} DB. Context Recall obtiene la puntuación más alta (0,85); Answer Relevancy la más baja (0,49).}
...
\section{Calidad de respuestas: RAGAS RAG}
\label{sec:res_ragas_rag}

Las 30~preguntas del \textit{golden set} RAG fueron procesadas sin errores en 1812,1~segundos. La Tabla~\ref{tab:res_ragas_rag} presenta los resultados.

\begin{table}[H]
\centering
\caption{Métricas RAGAS --- \textit{Golden Set} RAG (30~preguntas, 0~errores, 1812,1\,s).}
...
\caption{Métricas RAGAS del \textit{golden set} RAG. Faithfulness obtiene la puntuación más alta (0,57); las restantes métricas se sitúan en el rango 0,48--0,55.}
\label{fig:res_ragas_rag_barras}
\end{figure}

Las cuatro métricas RAG fueron inferiores a las obtenidas en el \textit{golden set} DB. Faithfulness (0,5708) fue la métrica más alta del conjunto RAG. Context Precision registró 0,5501 y Context Recall 0,4894.
...
\legend{DB (40 preg.), RAG (30 preg.)}
\end{axis}
\end{tikzpicture}%
}
\caption{Comparativa de métricas RAGAS entre los \textit{golden sets} DB y RAG.}
...
Se ejecutaron 51~mediciones (17~consultas $\times$ 3~ejecuciones) sin fallos en 102,2~segundos. La Tabla~\ref{tab:res_latencia} presenta las estadísticas por categoría.
...
\caption{Latencias por categoría: media, P95 y máximo. La categoría RAG presenta la mayor variabilidad (máx: 7,5\,ms).}
\label{fig:res_latencia_barras}
\end{figure}

La latencia media de todas las categorías se situó entre 2,0 y 2,4~ms. La categoría RAG registró la mayor variabilidad, con un máximo de 7,5~ms y un P95 de 4,1~ms. Las categorías VIZ y Complex presentaron las latencias más estables. Ninguna ejecución individual superó los 8~ms. Las 51~ejecuciones completaron con estado OK (0~fallos).
...
En el test SEC-PROMPT-003, el agente respondió: \textit{``No puedo hacer eso. Soy ChatHCE, un asistente especializado en análisis clínico de datos reales del dataset MIMIC-IV-ED. No puedo generar datos ficticios de pacientes.''}. El verificador automático marcó este test como no superado al detectar la palabra \textit{``datos''} en la respuesta de rechazo.
...
Los 8~casos RAG obtuvieron un score de categoría de 1,000. La Tabla~\ref{tab:res_tc_rag} detalla las puntuaciones individuales.
...
Seis casos alcanzaron puntuación perfecta con citación correcta de fuentes. TC-RAG-007 registró \texttt{herramienta\_correcta}=0,00: el agente respondió sin invocar explícitamente la herramienta RAG.
...
El criterio \texttt{herramienta\_correcta} registró 0,00 en los 5~casos de visualización. TC-VIZ-005 fue el único caso funcional que no superó su puntuación mínima individual.
...
TC-AGENT-002 & 1,000 & Selección multi-herramienta (DB + RAG) \\
...
\texttt{fuentes\_citadas}      & Citación de fuentes RAG    & 1,000 \\
...
\midrule
RAGAS DB       & 40 preguntas, 4 métricas   & 752,3\,s  & 1/4 métricas altas \\
RAGAS RAG      & 30 preguntas, 4 métricas   & 1812,1\,s & 4 métricas evaluadas \\
