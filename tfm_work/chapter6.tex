...
El criterio \texttt{no\_alucinacion} obtuvo puntuación perfecta (1,0) en los 28 casos de prueba funcionales, constituyendo el resultado más significativo para un sistema de asistencia clínica. El sistema reconoce explícitamente la ausencia de datos sin fabricar información, respondiendo con mensajes como ``El paciente con subject\_id 9999 no existe en el dataset \ac{MIMIC}-IV-\ac{ED}''.
...
\begin{itemize}
    \item \textbf{Dataset de demostración}: La evaluación se realizó sobre la versión demo de \ac{MIMIC}-IV-\ac{ED} (222 pacientes), que aunque mantiene patrones representativos, no permite evaluar el comportamiento del sistema a escala real.
...
\begin{description}
    ...
    \item[\ac{RAG} iterativo:] Exploración de técnicas de \ac{RAG} con queries de seguimiento \cite{xiong2024imedrag}, donde el sistema formula preguntas adicionales basándose en información recuperada previamente para resolver consultas complejas.
