...
\begin{figure*}[htbp]
\centering
\captionsetup{skip=0.9\baselineskip}
...
\node[toolnode]  (db)  at (-4.2,-7.1) {Database Tool\\$f_\text{DB}$};
\node[toolnode]  (rag) at ( 0.0,-7.1) {RAG Tool\\$f_\text{RAG}$};
\node[toolnode]  (viz) at ( 4.2,-7.1) {Visualization Tool\\$f_\text{VIZ}$};

\node[procnode, minimum width=5.5cm] (synth) at (0,-9.15) {Claude sintetiza respuesta integrada $\mathcal{A}(\cdot)$};
\node[ionode]   (out)   at (0,-10.75) {Respuesta + Gráficas interactivas + Fuentes};

\draw[flowarrow] (user)   -- (cache);
\draw[flowarrow] (cache)  -- (claude);
\draw[flowarrow] (claude) -- (select);

\coordinate (junc) at (0,-5.95);
...
\node[latency] at (6.35, -1.5)  {$\sim$5\,ms (hit)};
\node[latency] at (6.35, -3.0)  {P50: $\sim$800\,ms};
\node[latency] at (6.35, -7.1)  {P50: $\sim$1.5\,s / $\sim$2\,s / $\sim$3\,s};
\node[latency] at (6.35, -9.15) {P50: $\sim$1\,s};
\node[latency] at (6.35, -10.75) {Total P50: $<$5\,s};
...
\begin{tikzpicture}[
  layerbox/.style={
    ...
    minimum width=17cm, minimum height=2.45cm, inner sep=10pt,
    font=\small\sffamily
  },
  compbox/.style={
...
\node[layerlabel, anchor=north west] at ([xshift=8pt, yshift=-6pt]L1.north west) {Capa 1 --- Presentación};
...
% Interfaz 1-2
\node[ifacebox, minimum width=16.5cm] (I12) at (0, -1.75) {Interfaz: \texttt{UnifiedChatAgent.process\_message(message, context)} $\rightarrow$ \texttt{Dict[str, Any]}};

% Capa 2: Aplicación
\node[layerbox] (L2) at (0, -3.65) {};
\node[layerlabel, anchor=north west] at ([xshift=8pt, yshift=-6pt]L2.north west) {Capa 2 --- Aplicación};
\node[compbox, minimum width=3.8cm] at (-4.5, -3.70) {\texttt{unified\_agent.py}\\UnifiedChatAgent};
\node[compbox, minimum width=3.6cm] at (0.0, -3.70) {\texttt{prompt\_manager.py}\\PromptManager};
\node[compbox, minimum width=3.6cm] at (4.5, -3.70) {\texttt{llm\_manager.py}\\ClaudeLLMManager};

% Interfaz 2-3
\node[ifacebox, minimum width=16.5cm] (I23) at (0, -5.40) {Interfaz: \texttt{StructuredTool} \ac{API} (LangChain) $\mid$ \texttt{ClaudeToolAdapter.get\_langchain\_tool()}};

% Capa 3: Herramientas
\node[layerbox] (L3) at (0, -7.30) {};
\node[layerlabel, anchor=north west] at ([xshift=8pt, yshift=-6pt]L3.north west) {Capa 3 --- Herramientas};
\node[compbox, minimum width=3.2cm] at (-5.2, -7.35) {\texttt{database\_tool.py}\\DatabaseTool};
\node[compbox, minimum width=3.0cm] at (-1.5, -7.35) {\texttt{rag\_tool.py}\\RAGTool};
\node[compbox, minimum width=3.4cm] at (2.2, -7.35) {\texttt{viz\_collaboration.py}\\VisualizationTool};
\node[compbox, minimum width=3.2cm] at (6.0, -7.35) {\texttt{claude\_adapter.py}\\ClaudeToolAdapter};

% Interfaz 3-4
\node[ifacebox, minimum width=16.5cm] (I34) at (0, -9.05) {Interfaz: \texttt{DatabaseService.execute()}, \texttt{ImprovedRAGService.search()}, \texttt{VisualizationAgent.generate()}};

% Capa 4: Servicios
\node[layerbox] (L4) at (0, -10.95) {};
\node[layerlabel, anchor=north west] at ([xshift=8pt, yshift=-6pt]L4.north west) {Capa 4 --- Servicios};
\node[compbox, minimum width=2.9cm] at (-6.1, -11.00) {\texttt{database\_service.py}\\DatabaseService};
\node[compbox, minimum width=3.3cm] at (-2.7, -11.00) {\texttt{improved\_rag\_service.py}\\ImprovedRAGService};
\node[compbox, minimum width=3.1cm] at (0.9, -11.00) {\texttt{visualization\_agent.py}\\VisualizationAgent};
\node[compbox, minimum width=2.7cm] at (4.0, -11.00) {\texttt{cache\_manager.py}\\CacheManager};
\node[compbox, minimum width=2.7cm] at (7.0, -11.00) {\texttt{rate\_limiter.py}\\RateLimiter};

% Interfaz 4-5
\node[ifacebox, minimum width=16.5cm] (I45) at (0, -12.70) {Interfaz: \texttt{supabase-py} client $\mid$ \texttt{pgvector} RPC $\mid$ File I/O};

% Capa 5: Datos
\node[layerbox] (L5) at (0, -14.60) {};
\node[layerlabel, anchor=north west] at ([xshift=8pt, yshift=-6pt]L5.north west) {Capa 5 --- Datos};
\node[compbox, minimum width=4.0cm] at (-4.5, -14.65) {Supabase PostgreSQL\\esquema \texttt{mimic\_ed}};
\node[compbox, minimum width=4.0cm] at (0.5, -14.65) {Supabase pgvector\\tabla \texttt{rag\_chunks}};
\node[compbox, minimum width=3.5cm] at (5.2, -14.65) {File Storage\\Documentos clínicos};
