...
@pytest.mark.parametrize("tool_name", ["custom_query", "query_mimic_database", "execute_sql"])
async def test_legacy_free_form_query_entrypoints_are_not_registered(tool_name):
    registry = build_test_container().registry
    ctx = RequestContext(user_id="r", channel=Channel.api)
    result = await registry.dispatch(
        ctx,
        ToolUsePart(id=f"blocked-{tool_name}", name=tool_name, input={"query": "arbitrary"}),
    )
