...
async def test_parallel_tabs_same_session_keep_each_request_patient_scope():
    ...
    response_a, response_b = await asyncio.gather(
        container.chat_service.handle_chat(ChatRequest(message="labs A", session_id="shared-tab-session"), context_a, persist=False),
        container.chat_service.handle_chat(ChatRequest(message="labs B", session_id="shared-tab-session"), context_b, persist=False),
    )
