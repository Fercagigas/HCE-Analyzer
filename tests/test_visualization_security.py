"""Security and functional tests for deterministic visualizations."""
...
UI_PATH = PROJECT_ROOT / "ui" / "unified_chat_interface.py"


@pytest.fixture(scope="module")
def executor_module():
    """Load the isolated plotting module without initializing external services."""
    spec = importlib.util.spec_from_file_location(
        "visualization_code_executor_under_test",
        EXECUTOR_PATH,
    )
...
def test_timeseries_aliases_use_parameterized_plotter(
    executor_module,
    temporal_data,
    chart_type,
):
...
def test_bar_supports_preaggregated_category_values(executor_module):
    data = pd.DataFrame({
        "categoria": ["Diagnóstico A", "Diagnóstico B"],
        "frecuencia": [12, 7],
    })
...
def test_model_supplied_python_is_never_executed(
    executor_module,
    monkeypatch,
    tmp_path,
):
...
def test_requested_columns_and_parameter_sizes_are_validated(executor_module):
    ...
    missing_column = executor_module.create_allowlisted_visualization(
        "histogram",
        data,
    ...
    too_long_title = executor_module.create_allowlisted_visualization(
        ...
        data,
        metrics=["value"],
        title="x" * (executor_module.MAX_TITLE_LENGTH + 1),
    )
...
def test_runtime_has_no_dynamic_execution_calls():
    for path in (EXECUTOR_PATH, AGENT_PATH, UI_PATH):
        ...
        assert forbidden_calls == [], f"{path} contains {forbidden_calls}"
