...
_visualization_handler: Optional[VisualizationHandler] = None


def get_visualization_handler() -> VisualizationHandler:
    """
    Get global visualization handler instance (singleton).
    
    Returns:
        VisualizationHandler instance
    """
    global _visualization_handler
    if _visualization_handler is None:
        _visualization_handler = VisualizationHandler()
    return _visualization_handler


def process_and_display_response(
    response: Dict[str, Any],
    display_mode: str = "expander"
) -> Dict[str, Any]:
    """
    Process agent response and display visualizations.
    
    Args:
        response: Agent response dictionary
        display_mode: Display mode ("expander", "columns", "tabs", "inline")
        
    Returns:
        Processed response
    """
    handler = get_visualization_handler()
    processed_response = handler.process_agent_response(response)
    
    # Display visualizations if present
    visualizations = processed_response.get('visualizations', [])
    
    if visualizations:
        if display_mode == "expander":
            StreamlitVisualizationDisplay.display_with_expander(visualizations)
        elif display_mode == "columns":
            StreamlitVisualizationDisplay.display_in_columns(visualizations)
        elif display_mode == "tabs":
            StreamlitVisualizationDisplay.display_in_tabs(visualizations)
