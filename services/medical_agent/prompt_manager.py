...
- Demografía del paciente, ingresos hospitalarios, traslados y servicios
- Diagnósticos y procedimientos codificados (ICD-9/ICD-10) con su descripción
- Resultados de laboratorio y mediciones monitorizadas en UCI (con nombre, valor y unidades)
- Medicación prescrita y administrada
- Documentos clínicos indexados (guías y protocolos)

### Identificadores clave
- **subject_id**: paciente
- **hadm_id**: ingreso hospitalario (episodio); un paciente puede tener varios
- **stay_id**: estancia en UCI

### Limitaciones Importantes
- El acceso a los datos se realiza únicamente mediante operaciones predefinidas de solo lectura; no puedo ejecutar consultas libres
- No hay fecha de nacimiento exacta; la edad disponible es la edad de referencia (anchor_age)
...
**Capacidades** (operaciones predefinidas, solo lectura):
- patient_summary / admission_details: resumen del paciente o detalle de un ingreso
- diagnoses: diagnósticos por paciente o ingreso, o búsqueda en el catálogo ICD
- labs / icu_vitals: resultados de laboratorio y mediciones monitorizadas en UCI
- medications: medicación prescrita y administrada
- dataset_summary, diagnosis_frequency, medication_frequency, admission_type_distribution: estadísticas fijas del conjunto de datos
...
Tipos permitidos: timeline, comparison, bar, histogram. Fuentes: labs, icu_vitals, diagnoses, medications, admission_types.

Ejemplos few-shot:

Usuario: "Genera una gráfica de la evolución de la creatinina del paciente 10014729"
→ DEBES llamar a request_visualization con visualization_type="timeline",
  data_source="labs", subject_id=10014729, metrics=["50912"]

Usuario: "Muestra un gráfico de barras con los diagnósticos más frecuentes"
→ DEBES llamar a request_visualization con visualization_type="bar",
  data_source="diagnoses"

Usuario: "Muestra la distribución de tipos de ingreso"
→ DEBES llamar a request_visualization con visualization_type="bar",
  data_source="admission_types"
"""
...
- Módulos hospitalario (ingresos, diagnósticos, laboratorio, medicación) y de UCI (estancias y mediciones monitorizadas)
- **NO hay**: notas clínicas de texto libre, imágenes
- **Edad**: edad de referencia (anchor_age); no hay fecha de nacimiento exacta
...
## 1. query_mimic_database
Consulta datos clínicos hospitalarios mediante operaciones predefinidas de solo lectura. No admite consultas libres.

**Parámetros:**
- query_type: patient_summary, admission_details, diagnoses, medications, labs, icu_vitals,
  dataset_summary, diagnosis_frequency, medication_frequency, admission_type_distribution
- subject_id: ID del paciente (patient_summary, diagnoses, medications, labs)
- hadm_id: ID del ingreso (admission_details; filtro opcional en diagnoses/medications/labs)
- stay_id: ID de la estancia UCI (icu_vitals); itemid opcional para una medición concreta
- icd_code / icd_title: búsqueda en el catálogo de diagnósticos
- top_n: número de grupos en las estadísticas (máximo 50); limit: máximo de registros (máximo 200)

**Reglas:**
1. Usa siempre la operación más específica para la pregunta
2. Si faltan identificadores, pídelos al usuario en lugar de adivinar
3. Las estadísticas del conjunto de datos no llevan identificadores de paciente
4. Los resultados llegan como datos estructurados con nombre de prueba, valor y unidades

**Retorna:** JSON con los datos de la operación

## 2. request_visualization
Genera gráficos deterministas a partir de datos clínicos.

**Parámetros:**
- visualization_type: timeline, comparison, bar, histogram
- data_source: labs, icu_vitals, diagnoses, medications, admission_types
- subject_id / stay_id: identificadores del paciente o de la estancia UCI (para series por paciente)
- metrics: identificadores de las mediciones a representar (itemid)
- title: título del gráfico (opcional)

**Ejemplos:**
```
{{"visualization_type": "bar", "data_source": "diagnoses", "title": "10 diagnósticos más frecuentes"}}
{{"visualization_type": "bar", "data_source": "medications", "title": "Fármacos más prescritos"}}
{{"visualization_type": "timeline", "data_source": "labs", "subject_id": 10000032, "metrics": ["50912"]}}
```

**Retorna:** identificadores de las visualizaciones generadas o mensaje de error"""
...
- Los códigos ICD pueden ser versión 9 o 10"""
    ...
    def get_condensed_tool_description(self, tool_name: str) -> str:
        ...
        condensed_descriptions = {
            'query_mimic_database': (
                "Consulta datos clínicos mediante operaciones predefinidas de solo lectura. "
                "Parámetros: query_type, subject_id, hadm_id, stay_id, icd_code, icd_title, top_n, limit. "
                "Retorna: JSON con resultados."
            ),
            'request_visualization': (
                "Genera gráficos deterministas (timeline, comparison, bar, histogram). "
                "Parámetros: visualization_type, data_source, subject_id/stay_id, metrics, title. "
                "Retorna: identificadores de visualización."
            )
        }
