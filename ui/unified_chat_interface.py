...
the MIMIC-IV clinical database and RAG-indexed clinical documents through intelligent tool selection.
"""
...
class UnifiedChatInterface:
    ...
    def _initialize_unified_agent(self):
        ...
        try:
            self._unified_agent = StreamlitChatClient(get_container())
    ...
    def _show_sidebar(self):
        ...
        # Contexto clinico autorizado (ADR 0010 §2: el paciente activo lo fija el usuario, no el modelo)
        self._show_patient_context()
    ...
    def _show_patient_context(self):
        """Selector de paciente/episodio activo y modo investigacion (scope obligatorio)."""
        st.markdown("**🩺 Contexto clínico**")
        current_patient = st.session_state.get('patient_context')
        patient_text = st.text_input(
            "Paciente activo (subject_id)",
            value=str(current_patient) if current_patient else "",
            help="Las consultas clínicas solo devuelven datos del paciente activo. Déjalo vacío para preguntas generales.",
            key="patient_context_input",
        )
        patient_text = patient_text.strip()
        if patient_text and not patient_text.isdigit():
            st.warning("El identificador del paciente debe ser numérico")
            patient_text = ""
        new_patient = patient_text or None
        if new_patient != current_patient:
            st.session_state.patient_context = new_patient
            st.session_state.encounter_context = None

        if st.session_state.get('patient_context'):
            encounter_text = st.text_input(
                "Episodio (hadm_id, opcional)",
                value=str(st.session_state.get('encounter_context') or ""),
                key="encounter_context_input",
            ).strip()
            st.session_state.encounter_context = encounter_text if encounter_text.isdigit() else None
            st.caption(f"Paciente activo: {st.session_state.patient_context}")
        else:
            st.caption("Sin paciente activo: las herramientas clínicas se rechazan hasta que selecciones uno.")

        roles = set((st.session_state.get('user') or {}).get('roles', []))
        if 'researcher' in roles:
            st.session_state.research_mode = st.checkbox(
                "Modo investigación (estadísticas del conjunto de datos)",
                value=bool(st.session_state.get('research_mode', False)),
            )
        else:
    ...
    def _process_user_message(self, message: str):
        """
        Envia el mensaje al ChatService (via StreamlitChatClient) con el contexto de la sesion.
        La persistencia del turno y la clasificacion del analisis las hace el core.
        """
        logger.info(f"🤖 Processing user message: {message[:100]}...")

        with st.spinner("🤖 Procesando con el asistente clínico..."):
            try:
                user = st.session_state.get('user') or {}
                start_time = datetime.now()

                result = self._unified_agent.send(
                    message,
                    user_id=user.get('id') or 'anonymous',
                    roles=user.get('roles', []),
                    session_id=(st.session_state.get('current_session') or {}).get('id'),
                    patient_id=st.session_state.get('patient_context'),
                    encounter_id=st.session_state.get('encounter_context'),
                    research_mode=bool(st.session_state.get('research_mode', False)),
                    options=st.session_state.get('unified_config', {}),
                )

                response_time_ms = (datetime.now() - start_time).total_seconds() * 1000
                perf = st.session_state.setdefault('unified_performance', {'total_queries': 0, 'cache_hits': 0, 'total_response_time_ms': 0})
                perf['total_queries'] += 1
                perf['total_response_time_ms'] += response_time_ms

                # El ChatService crea la sesion si no existia: reflejarla en la UI
                session_id = result.get('session_id')
                if session_id and (st.session_state.get('current_session') or {}).get('id') != session_id:
                    st.session_state.current_session = {"id": session_id, "title": message[:57] + ("..." if len(message) > 57 else "")}

                if isinstance(result, dict) and result.get('success'):
                    result.setdefault('processing_time_ms', response_time_ms)
                    self._persist_visualizations_to_session(result)
                    st.session_state.unified_messages.append({"role": "assistant", "content": result})
                    ...
                    st.session_state.unified_messages.append({"role": "assistant", "content": error_response})
                ...
                error_response = {'success': False, 'content': f"❌ **Error de Sistema**\n\n{error_msg}", 'error': error_msg}
                st.session_state.unified_messages.append({"role": "assistant", "content": error_response})
                self._render_error_message(error_response)
    ...
    def _render_assistant_message(self, content):
        ...
        Handles:
        1. visualization_ids type - figuras Plotly (JSON) del repositorio del core
        ...
        if isinstance(content, dict):
            ...
            if content.get('visualizations') and st.session_state.unified_config.get('enable_visualizations'):
                ...
                for viz in content['visualizations']:
                    # visualization_ids: figuras del VisualizationRepository del core
    ...
    def _persist_visualizations_to_session(self, result: Dict[str, Any]) -> None:
        """Guarda las figuras (Plotly JSON) en st.session_state.viz_cache para sobrevivir a los reruns."""
        visualizations = result.get('visualizations', [])
        if not visualizations:
            return
        cache = st.session_state.setdefault('viz_cache', {})
        user_id = (st.session_state.get('user') or {}).get('id') or 'anonymous'
        for viz_entry in visualizations:
            ...
            for viz_id in viz_entry.get('ids', []):
                if viz_id in cache:
                    continue
                try:
                    figure_json = self._unified_agent.figure_json(user_id=user_id, viz_id=viz_id)
                except Exception as exc:  # noqa: BLE001
                    logger.warning(f"No se pudo recuperar la visualizacion {viz_id}: {exc}")
                    figure_json = None
                if figure_json:
                    cache[viz_id] = figure_json

    def _render_visualizations_from_store(self, viz_ids: list):
        """Renderiza figuras Plotly desde viz_cache (JSON) con st.plotly_chart."""
        ...
        for viz_id in viz_ids:
            figure_json = cache.get(viz_id)
            if figure_json is None and self._unified_agent is not None:
                try:
                    figure_json = self._unified_agent.figure_json(user_id=user_id, viz_id=viz_id)
                except Exception as exc:  # noqa: BLE001
                    logger.warning(f"Visualizacion {viz_id} no disponible: {exc}")
                if figure_json:
                    cache[viz_id] = figure_json
            if not figure_json:
                continue
            ...
            try:
                ...
                title = (figure.layout.title.text if figure.layout.title and figure.layout.title.text else "") or ""
                if title:
                    st.markdown(f"**{title}**")
                st.plotly_chart(figure, use_container_width=True, key=f"viz_{viz_id}")
            except Exception as exc:  # noqa: BLE001
                st.warning(f"⚠️ No se pudo renderizar la visualización {viz_id}: {exc}")
            st.markdown("---")
        if not found_any:
    ...
    def _get_tool_icon(self, tool_name: str) -> str:
        ...
        tool_icons = {
            'get_patient_summary': '🗄️', 'get_admission_details': '🏥', 'get_diagnoses': '🩺', 'get_labs': '🧪',
            'search_lab_items': '🔎', 'get_medications': '💊', 'get_icu_stays': '🛏️', 'get_icu_observations': '📈',
            'search_icd_codes': '📖', 'get_dataset_statistics': '📐',
            'search_clinical_documents': '📚', 'create_visualization': '📊',
            'query_mimic_database': '🗄️', 'request_visualization': '📊',
        }
...
def get_unified_chat_interface() -> "UnifiedChatInterface":
    """Instancia por sesion Streamlit (sin singleton de modulo: importar no toca st.session_state)."""
    if "unified_chat_interface" not in st.session_state:
        st.session_state.unified_chat_interface = UnifiedChatInterface()
    return st.session_state.unified_chat_interface


def show_unified_chat():
    """Main function to show unified chat interface"""
    get_unified_chat_interface().show()
